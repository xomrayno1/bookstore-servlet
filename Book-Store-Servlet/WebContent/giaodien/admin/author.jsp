		...
		<div class="row">
			<div class="col-sm-2">
				<div class="list-group">		
					...
					<a class="list-group-item active"  href='<c:url value="/admin/author"></c:url>'>Quản lý tác giả</a>
			...
			<div class="col-sm-10">
					...
					<c:forEach items="${listAuthorPagi}" var="item" varStatus="i">

						<tr>
							<td>${i.index + 1 + (pageCurrent - 1) * 6}</td>
					...
					<tr>
						<td colspan="7">
							...
							<li><a href='<c:url value="/admin/author?page=${pre}"></c:url>'>&laquo;</a>
								<c:forEach begin="1"  end="${soPage}" var="item">
									<c:if test="${pageCurrent != item}">
										<li  ><a href='<c:url value="/admin/author?page=${item}"></c:url>'>${item}</a></li>
									</c:if>
									<c:if test="${pageCurrent == item}">
										<li class="active" ><a href='<c:url value="/admin/author?page=${item}"></c:url>'>${item}</a></li>
									</c:if>
								...
								<li><a href='<c:url value="/admin/author?page=${next}"></c:url>'>&raquo;</a>
							</ul>
						</td>
					</tr>
					<tr>
